...
class ConnectionParser:
  ...
  def parse(self, region: Region):
    ...
    for entrance in region.entrances:
      ...
      print(f"begin parse at {entrance.default_connection.containing_region.name}")
      self.seen_entrances.append(entrance)
      self.parse(entrance.default_connection.containing_region)
      print('end parse')

def serialize_region_connections(region: Region, assigned_regions: dict[Region, str]) -> list[str]:
  ...
  for connecting_region, logic in region.region_connections.items():
    connecting_region_name = connecting_region.name
  ...
  for connecting_region, rule in region.region_connections.items():
    accum.append(f"# {region.name} -> {connecting_region.name}")
...
def assign_regions(regions: Iterable[Region]) -> tuple[list[str], dict[Region, str]]:
  region_id: int = 0
  str_ret: list[str] = []
  regs_ret: dict[Region, str] = {}
  for region in regions:
    str_ret.append(f"r{region_id}=R(\"{region.name}\",p,mw)")

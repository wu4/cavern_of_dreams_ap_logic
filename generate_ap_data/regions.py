...
class ConnectionParser:
    def parse(self, region: type[Region]):
        ...
        for entrance in region.entrances:
            if entrance in self.seen_entrances:
                continue
...
def serialize_region_connections(region: type[Region], assigned_regions: dict[type[Region], str]) -> list[str]:
    ...
    for connecting_region, rule in region.region_connections.items():
        accum.append(
            f"# {region_name(region)} -> {region_name(connecting_region)}")
        accum.append(f"{assigned_regions[region]}.connect({assigned_regions[connecting_region]},{
                     'None' if rule is None else 'lambda s:'+rule.into_server_code()})")
...
def generate():
    ...
    for region in parser.seen_regions:
        accum += indent(serialize_region_connections(region,
                        assigned_regions), 1)

...
def parse(location_datas: dict[str, FlagList]) -> Iterable[str]:
    ...
    for cat, flag_list in location_datas.items():
        ...
        accum += serialize_list(flag_list.items.values(),
                                f"{snake_case_cat}_items")

        if not isinstance(flag_list, FlagListWithLocations):
            continue

        all_locations.update(flag_list.locations)
        accum += serialize_list(flag_list.locations.values(),
                                f"{snake_case_cat}_locations")

    accum += serialize_list(all_locations.values(), "all_locations")
    accum += serialize_list(all_items.values(), "all_items")

    accum += serialize_dict({all_items[flag_name[9:]]: location_name for flag_name,
                            location_name in all_locations.items()}, "vanilla_locations")
